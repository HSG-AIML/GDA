    ...
    * flood mask single-channel png where no flood = 0, flood = 255
    ...
    def __init__(
        ...
        if split_file is not None:
            # load samples from pre-defined split file (e.g., new test/val split or few shot train splits)
            self.files = self._load_files_from_list(split_file)
        else:
            self.files = self._load_files(self.root, self.split)

    def _load_files_from_list(self, split_file):
        with open(split_file, "r") as f:
            files = [ast.literal_eval(l.strip()) for l in f.readlines()]
    ...
    def __getitem__(self, index: int):
        ...
        mask = flood_mask.unsqueeze(0)  # return only the flood mask
        # else:
        #    mask = water_mask.unsqueeze(0)

        vv = vv[0]  # rgb channels all contain the same data
        vh = vh[0]
        image = torch.cat(tensors=[vv.unsqueeze(0), vh.unsqueeze(0)], dim=0)
        sample = {"image": image, "mask": mask, "files": files}
        ...
        sample["mask"] = sample[
            "mask"
        ].squeeze()  # kornia transform needs batch dim, remove it again after applying the transform
    ...
    def _load_files(self, root: str, split: str):
